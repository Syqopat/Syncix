...
impl TreeBuilder {
    ...
    async fn process_directory(&self, dir_path: &Path) -> Result<Option<InstanceNode>, String> {
        ...
        // 1. Önce bu klasörün bir "Kök Dosyası" (init.*.json) var mı bul
            ...
            if self.vfs.is_file(path) {
                if let Some(filename) = path.file_name().and_then(|n| n.to_str()) {
                    if filename.starts_with("init.") && filename.ends_with(".json") {
                        // init.model.json gibi bir dosya bulundu
                        ...
                        // Not: JSON dosyasının içinde zaten class_name var. Herhangi bir serializer onu açabilir
